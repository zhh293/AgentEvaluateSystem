...
export default function SubmissionPage() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setState("BUILD QUEUED · controlled Compose/image build");
  ...
  return <section className="page reveal">
    <div className="page-heading compact"><div><span className="section-number">02—NEW RUN</span><h1>Submit an<br /><em>agent project.</em></h1></div><p>Compose-first for complete Agent systems.<br />Dockerfile remains available for one service.</p></div>
    <form className="submission-form" onSubmit={submit}>
      ...
      <fieldset><legend>04 / PROJECT</legend><label className="dropzone wide"><input type="file" accept=".zip,.tgz,.gz" onChange={e => setFile(e.target.files?.[0] ?? null)} /><b>{file ? file.name : "DROP OR SELECT PROJECT PACKAGE"}</b><span>ZIP · TAR.GZ · TGZ / agent-eval.yaml + Compose or Dockerfile / max 50 MB</span></label><p className="wide">Compose declares multi-service topology; Dockerfile remains the single-service option. <code>agent-eval.yaml</code> is always required and declares the entry service, lifecycle and invocation protocol.</p></fieldset>
